...
public class CarritoDAOCU04 implements iCarritoDAOCU04 {
    ...
    public String select(int id_carrito) {
        ...
        try (Connection con = Sql2oDAO.getSql2o().open()) {
            String sqlCarrito = "SELECT id_carritoRAEE, usuario_id FROM carritoraee WHERE id_carritoRAEE = :id_carrito";
            Carrito carrito = con.createQuery(sqlCarrito)
                    .addParameter("id_carrito", id_carrito)
                    .executeAndFetchFirst(Carrito.class);

            if (carrito == null) {
                return "{\"mensaje\": \"Carrito con id " + id_carrito + " no encontrado\"}";
            }
            ...
            logger.severe("Error al ejecutar consulta del carrito o productos: " + e.getMessage());
            e.printStackTrace();
